...
class DataLoader(object):
    ...
    def shuffle(self):
        ...
        if self._tgt_insts:
            paired_insts = list(
                zip(self._loc_insts, self._src_insts, self._tgt_insts))
            random.shuffle(paired_insts)
            self._loc_insts, self._src_insts, self._tgt_insts = zip(
                *paired_insts)
        else:
    ...
    def next(self):
        ...
        def convert_string_to_mat(adj_string):
            ...
            output = torch.Tensor(adj_string).view(dim,
                                                   dim)  # .type(torch.uint8)
        ...
        def pad_to_longest(insts, encoder=False):
            ...
            inst_position = np.array([
                [pos_i + 1 if w_i != Constants.PAD else 0 for pos_i, w_i in
                 enumerate(inst)]
