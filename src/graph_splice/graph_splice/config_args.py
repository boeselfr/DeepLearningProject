...
def get_args(parser):
    ...
    parser.add_argument(
        '-vi', '--validation_interval', type=int, default=4,
        dest='validation_interval', help='Per how many epoch to validate.')
...
def config_args(opt):
    ...
    opt.graph_root = path.join('/p/qdata/jjl5sw/ChromeGCN/processed_data/',
                               opt.cell_type, opt.window_size, 'hic')
    ...
    if opt.save_feats:
        ...
        if opt.name2 is not None:
            opt.model_name += '.' + opt.name2

    opt.model_name = path.join(opt.results_dir, opt.cell_type, opt.model_name)

    opt.dataset = path.join(opt.graph_data_root, opt.cell_type, opt.window_size)
