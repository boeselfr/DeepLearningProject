...
def finetune(graph_model, full_model, chromosomes, criterion, optimizer,
    ...
    if opt.adj_type in ['hic', 'both']:
        ...
        split_adj_dict = None
    print(f'{split} chromosomes: {chromosomes}.')
    desc_o = f'({split2desc[split]} on chromosomes)'
    for chromosome in tqdm(chromosomes, mininterval=0.5,
                           leave=False, desc=desc_o):

        tqdm.write(f'Reading in chromosome {chromosome} data.')
        temp_split = 'train' if split == 'train' else 'test'
        ...
        desc_i = f'({split2desc[split]} inner on chromosome {chromosome})'
